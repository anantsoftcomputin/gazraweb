...
import { Calendar, Users, Clock, Phone, Mail, Filter, Check, X, MessageSquare, Search } from 'lucide-react';
...
const AdminCafeBookings = () => {
  ...
  const applyFilters = () => {
    ...
    // Filter by date
    if (filters.date) {
      filtered = filtered.filter(booking => getBookingDate(booking) === filters.date);
    }
      ...
      if (result.success) {
        ...
        if (newStatus === 'confirmed' && booking) {
          const bookingDetails = {
            date: new Date(`${getBookingDate(booking)}T00:00:00`).toLocaleDateString('en-IN', { day: 'numeric', month: 'short', year: 'numeric' }),
              ...
              {filteredBookings.map((booking) => (
                <div key={booking.id} className="p-6 hover:bg-gray-50 transition-colors">
                      ...
                      <div className="flex items-start gap-4 mb-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            ...
                            <span className={`px-3 py-1 rounded-full text-xs font-medium border ${getStatusColor(booking.status || 'pending')}`}>
                          ...
                          <div className="grid md:grid-cols-2 gap-3 text-sm text-gray-600">
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {getBookingDate(booking)
                                ? new Date(`${getBookingDate(booking)}T00:00:00`).toLocaleDateString('en-IN', {
                                weekday: 'short',
                                year: 'numeric',
                                month: 'short',
                                day: 'numeric'
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {getBookingTime(booking) || 'Time not set'}
                            </div>
                            <div className="flex items-center gap-2">
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100 text-xs text-gray-500">
                    Submitted: {getCreatedAtDate(booking.createdAt).toLocaleString('en-IN')}
                  </div>
                </div>
              ))}
            </div>
