...
const AdminCafeTestimonials = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        const uploadResult = await uploadFile(imageFile, 'testimonials');
        ...
        if (editingTestimonial && editingTestimonial.image) {
          try {
            await deleteFile(editingTestimonial.imagePath || editingTestimonial.image);
          } catch (error) {
    ...
    if (window.confirm(`Are you sure you want to delete the testimonial from ${testimonial.name}?`)) {
      try {
        if (testimonial.image) {
          try {
            await deleteFile(testimonial.imagePath || testimonial.image);
          } catch (error) {
